...
namespace Application.Models
{
    public class DataBase
    {
        ...
        public int Insert(TaskView task)
        {
            ...
            command.CommandText =
                $"INSERT INTO Tasks(name, deadline, status) VALUES('{task.TaskName}', '{task.DeadlineText}', {(task.IsCompletedTask ? 1 : 0)})";
            command.ExecuteNonQuery();
            command.CommandText =
                $"SELECT ID FROM Tasks WHERE name = '{task.TaskName}' AND deadline = '{task.DeadlineText}' AND status = {(task.IsCompletedTask ? 1 : 0)}";
